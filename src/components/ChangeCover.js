            ...
            <div className="change-cover">
                <div className="cover-sort">
                    <span className="cover-sort-wrapper">
                        <div className="dropdown-title" onClick={() => this.handleButtonClick('main')}>
                            <span style={{marginRight: 5 + 'px'}}>{this.state.mainSelectedOption}</span>
                            <MdArrowDropDown size={20} />
                        </div>
                        { this.state.openMainCoverOptions && (
                        <div className="dropdown-menu">
                            ...
                            <div className="dropdown-list">
                                { mainOptions.map((option, i) => {
                                    return (
                                        <div className="dropdown-option" key={i} onClick={this.handleMainOptionSelection}
                                        style={{fontWeight: this.state.mainSelectedOption === option ? 'bold' : 'normal',
                                        color: this.state.mainSelectedOption === option ? 'white' : null
                                        }}>{option}</div>
                                    );
                                })}
                            </div>
                    ...
                    <span className="cover-sort-wrapper">
                        <div className="dropdown-title" onClick={() => this.handleButtonClick('other')}>
                            <span style={{marginRight: 5 + 'px'}}>{this.state.alternativeSelectedOption}</span>
                            <MdArrowDropDown size={20} />
                        </div>
                        { this.state.openAdditionalCoverOptions && (
                        <div className="dropdown-menu">
                            <svg width="24" height="8" viewBox="0 0 24 8" className="dropdown-triangle" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <title>icon</title>
                                <path fill="#53565d" d="M10.3359 1.1094C11.3436 0.437601 12.6564 0.437601 13.6641 1.1094L24 8L3.05823e-09 8L10.3359 1.1094Z"></path>
                            </svg>
                            <AlternativeOptions mainSelectedOption={this.state.mainSelectedOption} handleAlternativeOptionSelection={this.handleAlternativeOptionSelection} 
                            alternateOptionSelected={this.state.alternativeSelectedOption}/>
                        </div>
                        )}
                    </span>
                </div>
                <div className="change-view">
                    ...
                    <div style={{marginLeft: 16 + 'px'}}><img className="img" alt="img2" src="https://s.imgur.com/desktop-assets/desktop-assets/icon-play.aba7568f963e6376b7d1.svg"></img></div>
